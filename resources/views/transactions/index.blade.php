@extends('layouts.app', ['page' => __('Transactions'), 'pageSlug' => 'transactions', 'section' => 'transactions'])
                            ...
                            <tbody>
                                @foreach ($transactions as $transaction)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('transactions.type', ['type' => $transaction->type]) }}">{{ __($transactionname[$transaction->type]) }}</a>
                                        </td>
                                        ...
                                        <td class="td-actions text-right">
                                            @if ($transaction->sale_id)
                                                <a href="{{ route('sales.show', $transaction->sale) }}" class="btn btn-link" data-toggle="tooltip" data-placement="bottom" title="{{ __('More details') }}">
                                                    <i class="tim-icons icon-zoom-split"></i>
                                                </a>
                                            @elseif ($transaction->transfer_id)
                                                <a href="{{ route('transfer.show', $transaction->transfer) }}" class="btn btn-link" data-toggle="tooltip" data-placement="bottom" title="{{ __('More details') }}">
                                                    <i class="tim-icons icon-zoom-split"></i>
                                                </a>
                                            @else
                                                <a href="{{ route('transactions.edit', $transaction) }}" class="btn btn-link" data-toggle="tooltip" data-placement="bottom" title="{{ __('Edit Transaction') }}">
                                                    <i class="tim-icons icon-pencil"></i>
                                                </a>
                                                <form action="{{ route('transactions.destroy', $transaction) }}" method="post" class="d-inline">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="button" class="btn btn-link" data-toggle="tooltip" data-placement="bottom" title="{{ __('Delete Transaction') }}" onclick="confirm(' {{ __('Are you sure you want to delete this transaction?') }}') ? this.parentElement.submit() : ''">
